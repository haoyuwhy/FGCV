...
class FGVC(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.crossentropyLoss:
            l_pix = self.crossentropyLoss(self.output, self.lables)
            l_total += l_pix
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if use_pbar:
                pbar.update(1)
                pbar.set_description(f'Test {idx}, Acc {self.metric_results["acc"]/(idx+1)}, Loss {self.metric_results["loss"]/(idx+1)}')
        if use_pbar:
            pbar.close()

        self.metric_results["acc"] /= (idx + 1)
